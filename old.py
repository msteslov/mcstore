...
# async def cmd_start(message: types.Message):
#     all_users.add(message.from_user.id)
#     if Account.check_prof(message.from_user.id, message.from_user.username) and \
#        Account.add_username(message.from_user.id, message.from_user.username):
#         kb = admin_keyboard if message.from_user.id == ADMIN_ID else start_keyboard
#         await message.answer(
...
#             reply_markup=kb
#         )
#         await message.send(GROUP_ID, f'Зарегестрирован: {message.from_user.username}\nНик: ', message_thread_id = 153)
#     else:
#         await message.answer("Произошла ошибка при регистрации!")
...
async def cmd_bank_info(message: types.Message):
    # c_search = re.search(r'.(\w+)', message.text)
    # print(c_search)
    # if c_search:

    #     pool = Bank.trans_info(c_search.group(1))
    #     await message.reply(
    ...
    #     )
    #     # Добавьте обработку и вывод информации по необходимости
    ars = message.text.split(' ')
...
@dp.message(Command("пополнить", 'repl'))
async def cmd_repl(message: types.Message):
    """
    Возможны варианты:
    /пополнить -k [from_card_id] [to_card_id] [amount]  // перевод между своими счетами
    /пополнить [username] [amount] [сообщение]           // для админов, банкиров
    """
    ...
    if '-k' in args:
        if len(args) != 5:
            await message.answer(
                ...
                'Пример: /пополнить -k [from_card_id] [to_card_id] [amount]',
                parse_mode=ParseMode.MARKDOWN
            )
        else:
            from_card = args[2]
            to_card = args[3]
            amount = args[4]
            tr_id = Bank.top_up_card(from_card, to_card, message.from_user.id, amount)
            if tr_id:
                await message.answer(
                    f'Операция №{tr_id - 1} завершена.\n'
                    'Для получения подробной информации напишите /банк_инфо [номер]'
                )
                await bot.send_message(
                    "-1002480162505",
                    f'Транзакция №{tr_id - 1}\n'
                    f'from_card: {from_card}\n'
                    f'to_card: {to_card}\n'
                ...
                await message.answer('Возникла ошибка при проведении операции.')
    else:
        if len(args) < 3:
            await message.answer(
                'Неправильная форма ввода.\n'
                'Пример: /пополнить [username] [amount] [сообщение]',
                parse_mode=ParseMode.MARKDOWN
            )
        else:
            if Account.get_prof(message.from_user.id)['role'] not in ['bank', 'admin']:
                await message.answer('Недостаточно прав для этой команды.')
            else:
                user = args[1]
                amount = args[2]
                msg = ' '.join(args[2:])
                tr_id = Bank.top_up_adm(message.from_user.id, user, amount, msg)
                if tr_id:
                    await message.answer(
                        f'Операция №{tr_id - 1} завершена.\n'
                        'Для получения подробной информации напишите /банк_инфо [номер]'
                    ...
                    await bot.send_message(
                        ...
                        f'to: {user}\n'
                        f'ID получателя: {Account.get_id_by_usn(user)}\n'
                        f'Сумма: {amount}\n'
                        f'Сообщение: {msg}\n'
                        f'Тип: адм_пополнение',
                        message_thread_id=4
                    )
                else:
                    await message.answer('Возникла ошибка при проведении операции.')

# --- Обработка рабочих команд из исходного кода telebot --- #
